...
;scientists. This workshop follows examples in the book *[Statistics is Easy](https://www.amazon.com/Statistics-Second-Synthesis-Lectures-Mathematics/dp/160845570)* by 
...
;From the authors - Few people remember statistics and often have to refer to books to apply formulae,
...
;In this workshop we try to answer the questions for the results of an experiment, how accurate is
...
;by mistake
;- Resampling - Using bootstrapping from the assumed distribution and then to see how often the observed
;outcome materializes. The goal here is to estimate the probability that a sample comes from a particular 
; distribution
...
(defn render-one
  ...
  [color x-title y-title xy & {:keys [opacity] :or {opacity 1}}]
...
;The z test as listed [here](https://en.wikipedia.org/wiki/Binomial_test) without the continuity correction is 
...
; $$  (observation - \mu) / \sigma $$
  ...
  (let [successes 15
        mu  (* population initial-probability)
        sd  (Math/sqrt (* population initial-probability (- 1 initial-probability)))
        z (/ (- successes mu) sd) ]
    (ktest/p-value (ktest/test-result z (kstatsd/normal {:location 0.0 :scale 1.0}) )))
...
;- Is there a time component like a holiday or a time where people dont normally log on that could have benn used to explain this
...
;- Resampling - Randomly shuffling labels on the data, bootstrapping to calculate the size of the effect
...
#_(power (fn [exp-num] (drug-experiment 10000 drug placebo)))

;Can i reject the null hypothesis with 80% probability

#_(power (fn [exp-num] (drug-experiment 10000 drug placebo)) 0.8)
